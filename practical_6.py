...
class Airplane:
    ...
    def plane_info(self):
        """Print plane info"""
        info_str = f"Plane name: {self.plane_name}\n" \
                   f"Economy seats: {self.economy_seats - self.lock_economy_seats}\n" \
                   f"Business seats: {self.business_seats - self.lock_business_seats}\n" \
                   f"1st class seats:" \
                   f" {self.first_class_seats - self.lock_first_class_seats}\n"
        return info_str
...
def info_all(airplanes):
    """Print all planes info"""
    for i, temp in enumerate(airplanes):
        info = temp.plane_info()
        print(f'ID plane: {i}\n' + info)
...
def booking(airplanes, eco_zone, busi_zone, first_zone):
    ...
    book = Booking()
    info_all(airplanes)
    id_p = int(input('To reserve a seat, please input the ID of the required airplane: '))
    ...
    free_eco = airplanes[id_p].economy_seats - airplanes[id_p].lock_economy_seats
    free_busi = airplanes[id_p].business_seats - airplanes[id_p].lock_business_seats
    free_first = airplanes[id_p].first_class_seats - airplanes[id_p].lock_first_class_seats

    if zone == 1 and free_eco > 0:
        book.reserve_seat({"zone": eco_zone, "seat_number": free_eco})
        airplanes[id_p].lock_economy_seats += 1
    elif zone == 2 and free_busi > 0:
        book.reserve_seat({"zone": busi_zone, "seat_number": free_busi})
        airplanes[id_p].lock_business_seats += 1
    elif zone == 3 and free_first > 0:
        book.reserve_seat({"zone": first_zone, "seat_number": free_first})
        airplanes[id_p].lock_first_class_seats += 1
    else:
